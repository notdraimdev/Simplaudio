...
func _process(_delta: float) -> void:
	...
	if currentSaveTime < 0:
		...
		SplashStrings = ["Total listening time: %s!" % str(str(int(TimeSpentListening/60)/60 )
						...
						"debugging" if OS.has_feature("editor") else "Release build",
		...
		if DiscordRPC.get_is_discord_working():
			SplashStrings.append_array(["hello everybody my name is %s" % DiscordRPC.get_current_user()["username"],
									"%s is cooking" % DiscordRPC.get_current_user()["username"]])
